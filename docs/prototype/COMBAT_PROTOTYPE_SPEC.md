...
직업, 레벨업 카드, 드롭 장비, 영구 성장, 이야기 연출은 이번 버전에 넣지 않는다. 전투 검증을 방해하지 않도록 체력과 필살기 게이지는 정해진 규칙으로만 변한다.
...
멀티터치를 지원해 이동 중 점프, 회피 또는 스킬 사용이 가능해야 한다. 버튼 크기, 위치, 간격과 불투명도는 이후 설정 화면에서 조절할 수 있도록 좌표를 데이터로 분리한다.
...
5. 무기별 스킬과 공용 필살기 구현
6. 정예 전투, HUD, 효과음과 진동 추가
7. 실제 Android 기기 테스트와 수치 조정

다음 문서화 단계에서는 입력 상태도, 전투 데이터 스키마와 프로토타입 작업 티켓을 만든다.
