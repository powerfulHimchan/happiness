...
`CP-001`, `CP-002` 기술 확인과 `CP-101 입력 명령 계층`, `CP-102 캐릭터 지상 이동`, `CP-103 가변 점프`를 거쳐 `CP-104 지상 회피와 공중 대시`를 검증하는 프로젝트다. 현재 메인 화면에서는 지상 회피 무적 프레임과 방향 기반 공중 대시, 공중 1회 제한을 확인한다.
...
`.github/workflows/build-android-apk.yml`은 `game` 변경이 `main`에 반영될 때 ARM64 디버그 APK를 생성한다. 현재 결과물 이름은 `happiness-tale-cp104-evade-dash-apk`이며 APK와 SHA-256 파일을 14일간 보관한다.

이 APK는 개인 기기 테스트용 임시 디버그 키로 서명된다. 다음 빌드에서는 키가 달라질 수 있으므로 설치 충돌이 발생하면 기존 진단 앱을 삭제한 뒤 다시 설치한다. Google Play 배포에는 사용할 수 없다.

`CP-104` 기기 테스트에서는 아래를 확인한다.
...
다음 구현 티켓은 `CP-105 낙하와 안전 발판 복귀`다.
