...
`CP-001`, `CP-002` 기술 확인과 `CP-101~105` 이동 샌드박스, `CP-201~204` 대상·피해·검·활 전투를 거쳐 `CP-205 두 무기 전환`을 검증하는 프로젝트다. 현재 메인 화면에서는 0.50초 전환 제한, 0.20초 예약 입력과 검·활의 독립적인 공격 및 스킬 대기시간을 확인한다.
...
`.github/workflows/build-android-apk.yml`은 `game` 변경이 `main`에 반영될 때 ARM64 디버그 APK를 생성한다. 현재 결과물 이름은 `happiness-tale-cp205-weapon-swap-apk`이며 APK와 SHA-256 파일을 14일간 보관한다.

이 APK는 개인 기기 테스트용 임시 디버그 키로 서명된다. 다음 빌드에서는 키가 달라질 수 있으므로 설치 충돌이 발생하면 기존 진단 앱을 삭제한 뒤 다시 설치한다. Google Play 배포에는 사용할 수 없다.

`CP-205` 기기 테스트에서는 기존 이동·대상·피해·검·활 전투 항목과 함께 아래를 확인한다.
