...
`CP-001`, `CP-002` 기술 확인과 `CP-101~105` 이동 샌드박스, `CP-201~205` 전투·무기 전환을 거쳐 `CP-206 공용 필살기`를 검증하는 프로젝트다. 현재 메인 화면에서는 공격·회피 게이지 획득과 3초간 적 계열만 15퍼센트 속도로 느려지는 `새벽의 틈`을 확인한다.
...
`.github/workflows/build-android-apk.yml`은 `game` 변경이 `main`에 반영될 때 ARM64 디버그 APK를 생성한다. 현재 결과물 이름은 `happiness-tale-cp206-dawn-gap-apk`이며 APK와 SHA-256 파일을 14일간 보관한다.

이 APK는 개인 기기 테스트용 임시 디버그 키로 서명된다. 다음 빌드에서는 키가 달라질 수 있으므로 설치 충돌이 발생하면 기존 진단 앱을 삭제한 뒤 다시 설치한다. Google Play 배포에는 사용할 수 없다.

`CP-206` 기기 테스트에서는 기존 이동·대상·피해·검·활 전투 항목과 함께 아래를 확인한다.
...
- 붉은 훈련탄과 접촉하면 HP가 정확히 12 감소하고 0.50초 피격 무적이 적용되는가?
- 지상 회피 또는 공중 대시 중 훈련탄과 접촉하면 HP가 감소하지 않는가?
...
- 붉은 훈련탄을 지상 회피 또는 공중 대시 무적으로 통과하면 게이지가 12 증가하는가?
- 게이지가 100일 때 버튼이 `새벽 준비`로 강조되고 발동 후 0으로 소모되는가?
- `새벽의 틈`이 정확히 3초 동안 유지되고 적 표적과 붉은 훈련탄이 15퍼센트 속도로 움직이는가?
...
├── scripts/combat/training_enemy_projectile.gd
