...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-201` 자동 공격 대상 탐색과 표시 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: Godot 이동 액션 위에 전방·사거리·대상 유지 규칙과 선택 표식 구현 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 기존 이동·점프·회피·낙하 복귀를 유지하면서 전방 1.6m 안의 최근접 적 탐색, 20퍼센트 대상 전환 기준과 흰색 선택 표식을 확인하는 `CP-201` 전투 샌드박스입니다.
...
1. 오른쪽을 볼 때 왼쪽의 후방 표적이 선택되지 않는지 확인
2. 교차 표적 둘 사이에서 흰 표식이 빠르게 번갈아 바뀌지 않는지 확인
3. 반대 방향을 입력하면 즉시 새 전방 표적을 찾는지 확인
4. 표적에서 1.6m 이상 멀어지면 대상이 해제되는지 확인
5. `CP-202` 피해·피격 공통 처리 구현
