...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-202` 피해·피격 공통 처리 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: 대상 탐색 위에 공통 피해 이벤트, 중복 차단, 피격 무적과 사망 처리 구현 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 기존 이동·점프·회피·낙하·대상 탐색을 유지하면서 피해 이벤트, 동일 타격 중복 차단, 피격 후 0.50초 무적과 HP 0 사망을 확인하는 `CP-202` 전투 샌드박스입니다.
...
1. `피격 12`를 한 번 눌러 HP가 12 감소하고 0.50초 무적이 시작되는지 확인
2. 무적 시간 안에 다시 눌러 피해가 차단되는지 확인
3. 상태가 정상일 때 `중복 ×2`를 눌러 HP는 한 번만 감소하고 중복 차단 횟수가 증가하는지 확인
4. 지상 회피 직후 `피격 12`를 눌러 회피 무적으로 차단되는지 확인
5. `치명타`를 눌러 HP가 0이 되고 이동 입력이 잠기는지 확인
6. `CP-203` 검 기본 공격과 스킬 구현
