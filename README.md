...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-103` 가변 점프와 입력 보정 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: Godot CharacterBody2D 지상 이동·가변 점프 샌드박스 작성 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 `CP-102` 지상 이동을 유지하면서 짧게/길게 누르는 점프 높이, 코요테 타임과 착지 입력 버퍼를 확인하는 `CP-103` 점프 샌드박스입니다.
...
1. 점프 버튼을 짧게/길게 눌러 높이 차이 확인
2. 연습 발판 끝을 벗어난 뒤 0.10초 안에 눌러 `코요테` 표시 확인
3. 착지 0.12초 전에 눌러 `착지 버퍼` 표시 확인
4. 30/60 FPS에서 점프 높이와 조작 감각 비교
5. `CP-104` 지상 회피와 공중 대시 구현
