...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-206` 공용 필살기 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: 검·활 전투와 전환, 공용 필살기 `새벽의 틈` 게이지와 선택적 시간 감속 구현 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 공격 적중과 정확한 회피로 게이지를 채우고 `새벽의 틈`을 사용해 3초 동안 적과 적 투사체만 15퍼센트 속도로 느려지는지 확인하는 `CP-206` 전투 샌드박스입니다. 플레이어와 플레이어의 활 투사체는 정상 속도를 유지합니다.
...
1. 기본 공격 적중 시 4, 스킬 적중 시 8만큼 필살기 게이지가 오르는지 확인
2. 붉은 훈련탄을 회피 무적으로 통과하면 정확한 회피로 12 게이지가 오르는지 확인
3. 게이지 100에서 `새벽 준비` 버튼을 누르면 `새벽의 틈`이 3초간 유지되는지 확인
4. 발동 중 움직이는 표적과 붉은 훈련탄만 15퍼센트 속도가 되는지 확인
5. 같은 시간 동안 플레이어 이동과 플레이어의 활 투사체는 정상 속도인지 확인
6. `CP-206` 실기기 검증 후 `CP-301` 일반 적 세 종류 구현
