...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-102` 캐릭터 지상 이동 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: Godot CharacterBody2D 지상 이동 샌드박스 작성 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 `CP-101` 멀티터치 입력 계층을 유지하면서 가속·감속·방향 전환과 바라보는 방향을 확인하는 `CP-102` 지상 이동 샌드박스입니다.
...
1. 오른쪽으로 최고 속도까지 이동한 뒤 패드를 놓아 감속 PASS 확인
2. 오른쪽 이동 중 곧바로 왼쪽으로 전환해 방향 전환 PASS 확인
3. 30/60 FPS에서 같은 이동 거리와 정지 감각 확인
