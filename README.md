...
- 게임 기획서 버전: `0.1`
- 전투 프로토타입 명세 버전: `0.1`
- 단계: `CP-205` 두 무기 전환 실기기 검증판
- 주인공: 로안 / 루미
- 미소의 여왕: 세라
- 구현 소스: 검·활 전투, 0.50초 전환 제한, 입력 예약과 무기별 공격·스킬 대기시간 보존 구현 완료
...
Godot 4.7.2 stable에서 [`game/project.godot`](game/project.godot)를 엽니다. 현재 빌드는 검과 활을 오가며 0.50초 연속 전환 제한, 행동 종료 뒤 0.20초 전환 예약, 두 무기의 독립적인 기본 공격 대기시간과 스킬 재사용 대기시간을 확인하는 `CP-205` 전투 샌드박스입니다.
...
1. `활 전환`을 누르면 주 무기, 자동 공격 사거리와 두 스킬 버튼이 함께 바뀌는지 확인
2. 전환 버튼을 빠르게 반복해도 0.50초 동안 추가 전환이 차단되는지 확인
3. 검 스킬 사용 후 활로 바꾸고 다시 검으로 돌아와도 검 쿨다운이 초기화되지 않는지 확인
4. 활 스킬 사용 후 검으로 바꾸고 다시 활로 돌아와도 활 쿨다운이 초기화되지 않는지 확인
5. 각 무기의 자동 공격 직후 전환해도 HUD의 기본 공격 대기시간이 유지되는지 확인
6. 스킬 또는 회피 중 전환을 누르면 0.20초 안에 행동이 끝난 경우에만 예약 전환되는지 확인
7. `CP-206` 공용 필살기 구현
